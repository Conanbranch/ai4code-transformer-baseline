...
The code_max_length is a function of the number of sampled code cells. The default is ```code_max_length = int((self.total_max_len - self.md_max_len)/sample_size)```, where the code size (number of tokens per code cell) is fixed regardless of the number of code cells in the notebook. Use variable length code, where ```code_max_length = int((self.total_max_len - self.md_max_len)/num_samples)``` for a code size that is function of the actual number of code cells sampled (i.e. there was only 7 code cells but you were sampling 20, this gives you more tokens per code cell by giving you more tokens for code where only 7 code cells were sampled vs. where 20 code cells where sampled)':
