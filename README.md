...
The code_max_length is a function of the number of sampled code cells. The default is ```code_max_length = int((self.total_max_len - self.md_max_len)/num_samples)```, where the code size (number of tokens per code cell) is fixed regardless of the number of code cells in the notebook. Use variable length code, where ```code_max_length = int((self.total_max_len - self.md_max_len)/num_sampled)``` for code sizes that are a function of the actual number of code cells sampled (i.e. there was only 7 code cells (```num_sampled = 7```) but you were sampling 20 (```num_samples = 20```), this gives you more tokens per code cell where the number of code cells ```num_sampled < num_samples```):
